...
function createDeferred(timeout?: number): Deferred {
    ...
    if (timeout) {
        promise = Promise.race<Promise<boolean>>([new Promise((r) => setTimeout(r, timeout, true)), new Promise((r) => (resolve = r))]);
    } else {
...
function createWatchHost(
    ...
    { formatHost, parsedOptions, writeFile, status, resolveModule, transformers }: CreateProgramOptions,
    logError: any
    ...
    const baseHost = ts.createWatchCompilerHost(
        ...
        ts.sys,
        createProgram,
        buildDiagnosticReporter(ts, context, formatHost, logError),
        status,
...
export default function createWatchProgram(ts: typeof import('typescript'), context: PluginContext, options: CreateProgramOptions, logError) {
    return ts.createWatchProgram(createWatchHost(ts, context, options, logError));
}
